...
const DAILY_TASKS = ["paathram", "veg", "kari", "rice"];
const WEEKEND_TASKS = ["kitchen", "bathroom", "room"];
...
completionsRouter.put("/toggle", async (req, res, next) => {
    try {
        ...
        const validTasks = body.category === "daily" ? DAILY_TASKS : WEEKEND_TASKS;
        if (!validTasks.includes(body.taskId)) {
            throw new AppError(400, `Invalid task id: ${body.taskId}`);
        }
        const existing = await pool.query(`SELECT completed FROM task_completions
       WHERE task_date = $1::date AND category = $2 AND task_id = $3`, [body.date, body.category, body.taskId]);
        let completed;
        ...
        if (completed) {
            await pool.query(`INSERT INTO task_completions (task_date, category, task_id, completed, updated_at)
         VALUES ($1::date, $2, $3, TRUE, NOW())
         ON CONFLICT (task_date, category, task_id)
         DO UPDATE SET completed = TRUE, updated_at = NOW()`, [body.date, body.category, body.taskId]);
        }
        else {
            await pool.query(`INSERT INTO task_completions (task_date, category, task_id, completed, updated_at)
         VALUES ($1::date, $2, $3, FALSE, NOW())
         ON CONFLICT (task_date, category, task_id)
         DO UPDATE SET completed = FALSE, updated_at = NOW()`, [body.date, body.category, body.taskId]);
        }
        res.json({
            date: body.date,
