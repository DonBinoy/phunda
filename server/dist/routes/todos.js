...
todosRouter.get("/", async (req, res, next) => {
    ...
    `;
        ...
        const { rows: items } = await pool.query(`SELECT id, todo_list_id, title, completed, sort_order, created_at
       FROM todo_items
...
todosRouter.post("/", async (req, res, next) => {
    ...
    try {
        ...
        const { rows: listRows } = await client.query(`INSERT INTO todo_lists (title, person_id, task_date)
       VALUES ($1, $2, $3::date)
       RETURNING id, title, person_id, task_date, created_at`, [body.title, body.personId, body.date]);
        const list = mapList(listRows[0]);
        const items = [];
        for (let i = 0; i < body.items.length; i++) {
            const { rows } = await client.query(`INSERT INTO todo_items (todo_list_id, title, sort_order)
         VALUES ($1, $2, $3)
         RETURNING id, todo_list_id, title, completed, sort_order, created_at`, [list.id, body.items[i], i]);
...
todosRouter.put("/items/:id/toggle", async (req, res, next) => {
    try {
        ...
        const existing = await pool.query(`SELECT completed FROM todo_items WHERE id = $1`, [id]);
        ...
        const { rows } = await pool.query(`UPDATE todo_items SET completed = $2
       WHERE id = $1
       RETURNING id, todo_list_id, title, completed, sort_order, created_at`, [id, nextCompleted]);
...
todosRouter.delete("/:id", async (req, res, next) => {
    try {
        const id = z.string().uuid().parse(req.params.id);
        const result = await pool.query(`DELETE FROM todo_lists WHERE id = $1 RETURNING id`, [id]);
